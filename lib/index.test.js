...
describe('metalsmith-nestedlayouts', () => {
  it('should allow nesting a layout in another layout', done => {
    const base = path.join(process.cwd(), 'test', 'fixtures', 'nested-layout');
    const actual = path.join(base, 'build');
    const expected = path.join(base, 'expected');
    const metalsmith = new Metalsmith(base);
  ...
  it('should allow nesting a layout in nested layout', done => {
    const base = path.join(process.cwd(), 'test', 'fixtures', 'nested-layout-in-nested-layout');
    const actual = path.join(base, 'build');
    const expected = path.join(base, 'expected');
    const metalsmith = new Metalsmith(base);
  ...
  it('should properly render a nested layout inside src directory', done => {
    const base = path.join(process.cwd(), 'test', 'fixtures', 'nested-layout-inside-src');
    const actual = path.join(base, 'build');
    const expected = path.join(base, 'expected');
    const metalsmith = new Metalsmith(base);
    ...
    return metalsmith
      .use(plugin({ directory: 'src/layouts' }))
      .use((files, metal, next) => {
        Object.keys(files).forEach(key => {
          // eslint-disable-next-line no-param-reassign
          if (match(key, 'layouts/*')[0]) delete files[key];
        });
        next();
      })
      .build(err => {
  ...
  it('should process variables from the frontmatter in a layout', done => {
    const base = path.join(process.cwd(), 'test', 'fixtures', 'variables-layout');
    const actual = path.join(base, 'build');
    const expected = path.join(base, 'expected');
    const metalsmith = new Metalsmith(base);
  ...
  it('should override variables from the frontmatter in a layout with local ones', done => {
    const base = path.join(process.cwd(), 'test', 'fixtures', 'override-variables-layout');
    const actual = path.join(base, 'build');
    const expected = path.join(base, 'expected');
    const metalsmith = new Metalsmith(base);
